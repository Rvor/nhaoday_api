...
RSpec.describe Product, type: :model do
  ...
  describe ".filter_by_title" do
    before(:each) do
      @product01 = FactoryGirl.create :product, title: "A plasma TV"
      @product02 = FactoryGirl.create :product, title: "Fastest Laptop"
      @product03 = FactoryGirl.create :product, title: "CD Player"
      @product04 = FactoryGirl.create :product, title: "LCD TV"
    end

    context "when a 'TV' title pattern is send" do
      it "return the 2 products matching" do
        expect(Product.filter_by_title("TV").length).to eql 2
      end

      it "returns the product matching" do
        expect(Product.filter_by_title("TV").sort).to match_array([@product01, @product04])
      end
    end
  end
  ...
  describe ".above_or_equal_to_price" do
    before(:each) do
      @product01 = FactoryGirl.create :product, title: "A plasma TV", price: 100
      @product02 = FactoryGirl.create :product, title: "Fastest Laptop", price: 50
      @product03 = FactoryGirl.create :product, title: "CD Player", price: 150
      @product04 = FactoryGirl.create :product, title: "LCD TV", price: 99
    end

    it "returns the products which are above or equal to the price" do
      expect(Product.above_or_equal_to_price(100).sort).to match_array([@product01, @product03])
    end
  ...
  describe ".below_or_equal_to_price" do
    ...
    it "returns the products which are above or equal to price" do
      expect(Product.below_or_equal_to_price(99).sort).to match_array([@product02, @product04])
    end
  ...
  describe ".recent" do
    ...
    it "returns the most update records" do
      expect(Product.recent).to match_array([@product03, @product02, @product04, @product01])
    end
